...
num_heketi_pods_after=$(kubectl get pods | grep -s "heketi-" | grep -vs "heketi-deploy" | wc -l)

if (( num_heketi_pods_after - num_heketi_pods_before != 3 )); then
